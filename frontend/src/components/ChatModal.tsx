  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      <div
        ...
        className="w-full max-w-xl bg-[#fdfae8] border-3 border-black rounded-3xl shadow-[8px_8px_0px_#000] overflow-hidden my-6 h-145 flex flex-col text-left animate-in fade-in zoom-in-95 duration-150"
      >
        {/* Chat Header */}
        <div className="px-5 py-3.5 border-b-2 border-black flex items-center justify-between bg-[#fecd0e] shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#131d27] text-white flex items-center justify-center font-black text-sm border-2 border-black shadow-[2px_2px_0px_#000]">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="w-4 h-4 rounded-full bg-[#fee26d] border border-black flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-xl border-2 border-black bg-white hover:bg-[#fee26d] text-black shadow-[2px_2px_0px_#000] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all cursor-pointer"
        ...
        {activeToolTitle && (
          <div className="px-5 py-2.5 bg-[#fee26d] border-b-2 border-black flex items-center justify-between text-xs font-bold text-black shrink-0">
        ...
        {/* Messages List Area */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 bg-[#fdfae8]">
          {currentThread.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6">
              <div className="w-12 h-12 rounded-2xl bg-[#fecd0e] border-2 border-black shadow-[3px_3px_0px_#000] flex items-center justify-center text-black mb-3">
                ...
                >
                  <div
                    className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-xs sm:text-sm font-semibold leading-relaxed border-2 border-black shadow-[2.5px_2.5px_0px_#000] ${
                      isMe
                        ? "bg-[#fecd0e] text-black rounded-br-xs"
        ...
        <div className="px-4 py-2 bg-white border-t-2 border-black overflow-x-auto flex gap-2 scrollbar-none shrink-0">
          {QUICK_CHIPS.map((chip, idx) => (
            <button
              ...
              className="text-[11px] font-bold whitespace-nowrap px-3 py-1.5 rounded-full bg-[#fdfae8] border-2 border-black text-black hover:bg-[#fee26d] shadow-[2px_2px_0px_#000] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all shrink-0 cursor-pointer"
            >
              {chip}
            </button>
          ))}
        ...
        <form
          onSubmit={handleSend}
          className="p-3 sm:p-4 bg-[#fdfae8] border-t-2 border-black flex items-center gap-2 shrink-0"
        >
          <input
            type="text"
            value={inputMessage}
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl border-2 border-black bg-[#fecd0e] hover:bg-[#fee26d] text-black text-xs font-black shadow-[2px_2px_0px_#000] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5 shrink-0 cursor-pointer"
          >
